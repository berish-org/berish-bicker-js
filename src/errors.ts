...
export const DependencyListDeclarationIsNotEqualsCorrect = (hookName?: string) =>
  Error(['DependencyList is not equals by prev depsList', hookName && ` for '${hookName}' hook`].filter(Boolean).join(''));

export const HookWrongCall = (hookName?: string) => Error(['Wrong call hook', hookName && ` with '${hookName}' hookName`].filter(Boolean).join(''));
